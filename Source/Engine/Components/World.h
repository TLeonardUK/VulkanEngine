...
#include "Engine/Components/MessageQueue.h"
...
public:
	...
	ComponentType* AddComponent(Entity entity)
	{
		...
		auto componentIter = state.components.find(typeid(ComponentType));
		...
		state.components.emplace(typeid(ComponentType), index);
		...
		return reinterpret_cast<ComponentType*>(pool.GetIndexPtr(index));
	}
